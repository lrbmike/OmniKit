...
| 工具名称 | 英文名称 | 图标 | 组件路径 |
|---------|---------|------|---------|
| 文本翻译 | Text Translation | `Languages` | `translator` |
| 变量名生成器 | Variable Name Generator | `Code2` | `var-name-generator` |
...
- **AI 提供商管理** (`/admin/settings/ai-providers`) - 配置和管理 AI 提供商
- **翻译器设置** (`/admin/settings/ai`) - 配置翻译器使用的 AI 提供商
- **变量名生成器设置** (`/admin/settings/var-name-gen`) - 配置变量名生成器使用的 AI 提供商
...
#### 9.2 Phase 2 - 其余工具（已完成 ✅）
7. URL 编解码 (URL Encoder)
8. Markdown 预览 (Markdown Preview)
9. 正则表达式测试 (Regex Tester)
10. 时间戳转换 (Timestamp Converter)
11. UUID 生成器 (UUID Generator)
12. JWT 解析 (JWT Decoder)
13. 渐变生成器 (Gradient Generator)
14. 颜色对比检测 (Contrast Checker)
15. 图片压缩 (Image Compressor)
16. Base64 图片转换 (Image to Base64)
17. 文本差异对比 (Text Diff)
18. 字数统计 (Word Counter)

#### 9.3 Phase 3 - AI 工具（已完成 ✅）
19. 文本翻译 (Text Translation)
20. 变量名生成器 (Variable Name Generator)
...
#### 10.6 AI 集成系统
- ✅ AI 提供商管理系统
- ✅ 支持多个 AI 提供商配置（OpenAI、Deepseek、Ollama 等）
- ✅ 每个 AI 工具可独立选择提供商
- ✅ 自定义系统提示词
- ✅ AI 驱动的文本翻译工具
- ✅ AI 驱动的变量名生成工具
...
- [x] 18 个工具清单是否需要调整？ (已确认)
- [x] 优先实现 6 个核心工具的方案是否同意？ (已完成)
- [x] 多用户支持"预留设计，暂不实现"的方案是否同意？ (已确认)
